...
function page_links(int $page,int $pages,string $base):void{if($pages<2)return;echo '<div class="pagination">';for($i=1;$i<=$pages;$i++){if($i===1||$i===$pages||abs($i-$page)<=2){$u=$base.(str_contains($base,'?')?'&':'?').'page='.$i;echo '<a href="'.e($u).'"'.($i===$page?' aria-current="page"':'').'>'.$i.'</a>';}}echo '</div>';}
